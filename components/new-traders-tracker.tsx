  ...
  const handleApply = async () => {
    try {
      ...
      if (timeRange === 'today') {
        ...
        endDateStr = getDateString(new Date());
      } else if (timeRange === 'week') {
        startDateStr = getDateString(getDaysAgo(7));
        endDateStr = getDateString(getDaysAgo(1));
      ...
      console.log(`[v0] Analyzing dates: ${allDates.join(', ')}`);

      // Fetch all date data
      const dataByDate: Record<string, any[]> = {};
      for (const date of allDates) {
        const data = await fetchTraderData(date);
        if (data) {
      ...
      if (Object.keys(dataByDate).length === 0) {
        setErrorMessage('No data available for selected date range');
        setActiveTraders([]);
        return;
      }

      // Analyze all possible 2+ day combinations with async batching
      const activeTraderMap = new Map<string, { userId: string; address: string; volChange: number; oldVol: number; newVol: number }>();
      const dates = Object.keys(dataByDate).sort();

      // First pass: track all traders across all dates to find earliest and latest
      const traderFirstLast = new Map<string, { firstDate: string; lastDate: string }>();
      
      dates.forEach((date) => {
        dataByDate[date].forEach((trader: any) => {
          if (!traderFirstLast.has(trader.address)) {
            traderFirstLast.set(trader.address, { firstDate: date, lastDate: date });
          } else {
            const entry = traderFirstLast.get(trader.address)!;
            entry.lastDate = date; // Since dates are sorted, this will be the latest
          }
        });
      });

      // Second pass: calculate volume change from first to last date for each trader
      traderFirstLast.forEach((dateRange, address) => {
        const firstDateData = dataByDate[dateRange.firstDate];
        const lastDateData = dataByDate[dateRange.lastDate];

        const firstTrader = firstDateData.find((t: any) => t.address === address);
        const lastTrader = lastDateData.find((t: any) => t.address === address);

        if (firstTrader && lastTrader) {
          const firstVol = parseFloat(firstTrader.vol) || 0;
          const lastVol = parseFloat(lastTrader.vol) || 0;
          const volChange = lastVol - firstVol;

          if (volChange !== 0) {
            activeTraderMap.set(address, {
              userId: firstTrader.userId || lastTrader.userId,
              address,
              volChange,
              oldVol: firstVol,
              newVol: lastVol,
      ...
      const active = Array.from(activeTraderMap.values())
        .sort((a, b) => Math.abs(b.volChange) - Math.abs(a.volChange));
      
      console.log(`[v0] Found ${active.length} active traders across all combinations`);
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4 bg-card border border-border">
        ...
        <div className="space-y-4">
          ...
          {/* Apply Button */}
          <Button onClick={handleApply} disabled={loading} className="w-full h-8 text-xs bg-orange-500 hover:bg-orange-600 text-white">
          ...
          {loading && (
            <div className="p-3 bg-orange-500/10 border border-orange-500/50 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs text-orange-600">Analyzing data across all date combinations... This may take a moment.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {activeTraders.length > 0 && (
        <Card className="p-4 bg-card border border-border">
          <div className="mb-4">
            <p className="text-xs md:text-sm font-semibold text-orange-500">
