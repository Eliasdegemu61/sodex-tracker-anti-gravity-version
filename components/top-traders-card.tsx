  ...
  return (
    <Card className="p-5 bg-card/20 backdrop-blur-xl border border-border/20 rounded-3xl shadow-sm group">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="px-2 py-0.5 rounded-lg bg-orange-500/10 text-orange-400 text-[8px] font-mono font-bold uppercase tracking-widest">Perps</div>
      </div>
          ...
          traders.map((trader: PerpTrader, idx: number) => (
            <div key={trader.address} className="flex items-center justify-between p-3 bg-secondary/5 rounded-2xl border border-border/5 hover:bg-orange-500/5 transition-all duration-300">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-[11px] font-mono text-foreground/60 dark:text-foreground/60 text-foreground/80 truncate">{formatAddress(trader.address)}</span>
              </div>
