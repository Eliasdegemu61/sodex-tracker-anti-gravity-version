...
class CacheManager {
  ...
  set<T>(key: string, data: T): void {
    ...
    // Persist to localStorage on client side (skip if key contains 'leaderboard' - data too large)
    if (this.isClient && !key.includes('leaderboard')) {
      try {
        ...
        // Data is still available in memory and from Redis on server
      }
    }
  }
  ...
  get<T>(key: string): T | null {
    ...
    // If not in memory but on client, try to restore from localStorage (skip leaderboard - data too large)
    if (!entry && this.isClient && !key.includes('leaderboard')) {
      try {
